...
interface TemplateSettings {
  colorTheme: string; // 'amethyst', 'city', 'flat', 'modern', 'smooth'
  ...
  mainContent: string; // 'boxed', 'narrow', ''
}
...
interface SidebarMiniPayload {
  mode: 'on' | 'off' | 'toggle';
}

interface SidebarPositionPayload {
  mode: 'left' | 'right' | 'toggle';
}

interface SidebarStylePayload {
...
interface HeaderPayload {
  mode: 'fixed' | 'static' | 'toggle';
}

interface HeaderStylePayload {
  mode: 'dark' | 'light' | 'toggle';
}
...
interface HeaderLoaderPayload {
  mode: 'on' | 'off' | 'toggle';
}

interface PageLoaderPayload {
  mode: 'on' | 'off' | 'toggle';
}

interface MainContentPayload {
  mode: 'full' | 'boxed' | 'narrow';
}
...
interface ColorThemePayload {
  theme: string;
}

interface SideTransitionsPayload {
  transitions: boolean;
}
...
export const useTemplateStore = defineStore("template", {
  ...
  actions: {
    ...
    setLayout(payload: LayoutOptions): void {
    ...
    sidebar(payload: SidebarPayload): void {
    ...
    sidebarMini(payload: SidebarMiniPayload): void {
      if (window.innerWidth > 991) {
        if (payload.mode === "on") {
    ...
    sidebarPosition(payload: SidebarPositionPayload): void {
    ...
    sidebarStyle(payload: SidebarStylePayload): void {
    ...
    sideOverlay(payload: OverlayPayload): void {
      if (payload.mode === "open") {
    ...
    sideOverlayHover(payload: OverlayHoverPayload): void {
      if (payload.mode === "on") {
    ...
    pageOverlay(payload: PageOverlayPayload): void {
      if (payload.mode === "on") {
    ...
    header(payload: HeaderPayload): void {
    ...
    headerStyle(payload: HeaderStylePayload): void {
      if (payload.mode === "dark") {
    ...
    headerSearch(payload: HeaderSearchPayload): void {
      if (payload.mode === "on") {
    ...
    headerLoader(payload: HeaderLoaderPayload): void {
      if (payload.mode === "on") {
    ...
    pageLoader(payload: PageLoaderPayload): void {
      if (payload.mode === "on") {
    ...
    mainContent(payload: MainContentPayload): void {
    ...
    // Set Dark Mode
    darkMode(payload: DarkModePayload): void {
    ...
    setColorTheme(payload: ColorThemePayload): void {
      const lHtml = document.documentElement;
    ...
    setSideTransitions(payload: SideTransitionsPayload): void {
